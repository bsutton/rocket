...
class _ValueParser extends Parser {
  final _expected = [
    _openBrace.name,
    _openBracket.name,
    'string',
  ...
  Tuple1? parse(ParseState state) {
    ...
    switch (c) {
      case $open_brace:
        return _object.parse(state);
      case $open_bracket:
        return _array.parse(state);
      case $quote:
        return _string.parse(state);
      case $f:
        return _false.parse(state);
      case $t:
        return _true.parse(state);
      case $n:
        return _null.parse(state);
      default:
        if (c >= $0 || c <= $9 || c == $minus || c == $plus) {
          return _number.parse(state);
        }
    }

    state.failAll(_expected, state.pos);
  }
}
